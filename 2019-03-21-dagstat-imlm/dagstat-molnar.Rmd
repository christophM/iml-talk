...
Use only interpretable models (e.g. linear model)! $\Rightarrow$ Pay with loss in performance.

```{r, echo = FALSE}
knitr::include_graphics("../images/white-box.png")
```
## Problem: Tradeoff interpretability and performance
Just use post-hoc interpretation methods (e.g. feature importance)! $\Rightarrow$ Works better for less complex models.
...
TODO of fuzzy animal which dodges touch of hand
Or use image of cat that doges hand

## We propose

TODO: Replace text with image: Black box in the middle, three properties attached to it
...
```
## Measure: Main effect complexity

TODO: Plot of single ALE plot with complexity (from paper)

Plus formula how to average those for all plots

## Measure: Interaction Strength

Measure main effect model with proportion of error explained (pre):


$$\text{Interaction Strength} =  \frac{\mathbb{E}(L(\hat{f}, f_{ALE}))}{\mathbb{E}(L(\hat{f}, c))}$$
