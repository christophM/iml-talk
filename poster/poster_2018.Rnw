...
<<feature-importance, echo=FALSE, fig.width=12, fig.height=3>>=
imp = FeatureImp$new(pred, "mae")

impres = imp$results

ggplot(impres) + geom_col(aes(y = importance, x = feature)) + coord_flip() +
  theme(aspect.ratio=1/4, text = element_text(size=20),
...
<<ice, echo=FALSE, fig.width=12, fig.height=3>>=
...
Suppose the rent model predicts \Sexpr{pred.rent} Euros rent, for an appartment in the outer city with a living area of 92 $m^2$ and where cats are not allowed. 
How can we explain this prediction?


<<counterfact, echo=FALSE, include = FALSE>>=
appartment.mod = appartment
appartment.mod$cat.allowed = factor("Yes", levels = c("No", "Yes"))
...
For example: How does the input have to change so that the prediction changes above 1100 Euros?
...
<<shapley, echo=FALSE, fig.width=12, fig.height=3>>=
...
Some models are intrinsically interpretable, because they learn simple relationships between input and output.
Their big disadvantage is that they often predict the data less well.
...
$x_1$ to $x_p$ are the features that are used to predict the outcome y. 
The goal is to find the best possible weights $\beta$.
In our rent example, the best weights are:
...
$$\text{rent} = \Sexpr{cf["(Intercept)"]} + \Sexpr{cf["size"]} \cdot \text{size}  \Sexpr{cf["locationOuter city"]} \cdot I_{\text{outer city}} +  \Sexpr{cf["cat.allowedYes"]} \cdot I_{\text{cats allowed}}$$
