...
Machine learning can be used to predict credit default, risk for cancer, weekly sales ...
}
...
Interpretable machine learning is a set of methods and models that enable to explain the behaviour and predictions of machine learning systems.
}
...
A method called Shapley Values fairly attributes the difference between the prediction and the average prediction among the feature values to explain a prediction.
