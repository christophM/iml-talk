...
plot(shap) +
  theme(aspect.ratio=2/4, text = element_text(size=30),
    panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  scale_y_continuous("Contribution to prediction minus avg. prediction") +
  scale_x_discrete("")
...
The idea behind local surrogate models \cite{ribeiro2016should} is to replace the complex model with an individual, locally weighted interpretable model (e.g. decision tree) for the data point for which we want to explain the prediction.
