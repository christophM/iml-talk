...
Machine learning is a set of methods that let a computer learn from data to make predictions (e.g. cancer, weekly sales, credit default) by finding patterns.
<<ml>>=
...
Solution: \textbf{Interpretable machine learning} is a set of methods and models that enable to explain the behaviour and predictions of machine learning systems.\cite{molnar}
}
...
As example we train a machine learning model (here a random forest) that predicts the rent of an appartment given the size of its living area, the location ("Good or "Bad") and if cats are allowed.
...
The feature effect describe how changing a features changes the prediction of all data points on average. \cite{friedman2001greedy}
...
@
\textit{For example we can vary the size of the living area and observe how the prediction of the model changes.}
...
The feature importance \cite{Fisher2018} tells us how much the model accuracy drops when we don't know a feature, which is simulated by shuffling the feature values.
...
@
\textit{In the rent example the most important feature was the size of the living area. }
}
...
Answer (one of many possible ones): \\
\textit{If cats were allowed, the predicted rent would be \Sexpr{pred.new} Euros.}
\\ \\
\textbf{Shapley Values} \\
From a game theoretical perspective, the feature values can be seen as players in a game that cooperate to achieve the prediction as a payout.
The Shapley Values method \cite{vstrumbelj2014explaining} fairly attributes the difference between the prediction and the average prediction among the feature values.
...
but often predict the data less well than flexible, yet less interpretable, models.
\\ \\
...
@
\textit{The tree predicts a rent of 1908 Euros for an appartment bigger than 77 $m^2$ in a good location.}
\\ \\
\textbf{Decision Rules} \\
Decision rules are IF-THEN statements consisting of a condition and a prediction.
One or many rules can be used to make predictions. For example:
...
Intrinsically interpretable models (e.g. tree)  can also be used as to increase interpretability of a black box model (e.g. neural network) by approximating its predictions, acting as a surrogate model which can be interpreted. 
<<surrogate>>=
knitr::include_graphics("figure/global-surrogate.png")
@
\\ \\
\textbf{Local Surrogate Models}\cite{ribeiro2016should} \\
Replace a  complex model with an individual, locally weighted interpretable model (e.g. decision tree) for the data point for which we want to explain the prediction.

<<lime, echo=FALSE, out.width="0.2\\textwidth">>=
